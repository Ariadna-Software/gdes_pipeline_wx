...
var ofertaHitosDb = require("./ofertas-hitos.db_mysql");
...
router.get('/oferta/:ofertaId', function (req, res) {
    ...
    if (!ofertaId) return res.status(400).send('Debe indicar el identificador de ofertaHito');
    ofertaHitosDb.getByOfertaId(ofertaId, function (err, ofertaHitos) {
        if (err) return res.status(500).send(err.message);
        res.json(ofertaHitos);
    });
})
