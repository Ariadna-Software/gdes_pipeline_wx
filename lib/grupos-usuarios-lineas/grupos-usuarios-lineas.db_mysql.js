...
var gruposUsuariosLineasDbAPI = {
    ...
    getById: function (id, done) {
        comun.getConnectionCallback(function (err, con) {
            if (err) return done(err);
            var sql = "SELECT * FROM grupos_usuarios WHERE grupoUsuarioLineaId = ?";
    ...
    put: function (grupoUsuariosLineas, done) {
        comun.getConnectionCallback(function (err, con) {
            if (err) return done(err);
            var sql = "UPDATE grupos_usuarios SET ? WHERE grupoUsuarioLineaId = ?";
    ...
    delete: function (id, done) {
        comun.getConnectionCallback(function (err, con) {
            if (err) return done(err);
            var sql = "DELETE FROM grupos_usuarios WHERE grupoUsuarioLineaId =  ?";
            sql = mysql.format(sql, id);
            con.query(sql, function (err, grupo) {
                con.end();
                if (err) return done(err);
