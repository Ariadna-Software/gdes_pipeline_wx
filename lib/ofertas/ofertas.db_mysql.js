...
var ofertasDbAPI = {
    ...
    getOfertasUsuario: function (id, gid, gid2, gid3, gid4, gid5, gid6, gid7, done) {
        comun.getConnectionCallback(function (err, con) {
            if (err) return done(err);
            var sql = "SELECT o.*, "
            sql += " a.nombre AS nomArea, un.nombre AS nomUnidadNegocio, e.nombre AS nomEmpresa, p.nombre AS pais,";
            ...
            sql += " LEFT JOIN servicios AS ser ON ser.servicioId = o.servicioId";
            sql += " LEFT JOIN usuarios AS u2 ON u2.usuarioId = o.responsableId";
            sql += " WHERE o.usuarioId = ?";
            sql += " OR o.usuarioId IN (SELECT usuarioId FROM grupos_usuarios_lineas WHERE grupoUsuarioId = ?)";
            sql += " OR o.ofertaId IN (SELECT ofertaId FROM seguidores WHERE usuarioId = ?)";
            sql += " OR o.ofertaId IN (SELECT ofertaId FROM ofertas WHERE responsableId = ?)";
            sql += " OR o.ofertaId IN (SELECT ofertaId FROM ofertas WHERE usuResponsableId = ?)";
            if (gid2) {
                sql += " OR o.usuarioId IN (SELECT usuarioId FROM grupos_usuarios_lineas WHERE grupoUsuarioId = + " + gid2 + ")";
            }
            if (gid3) {
                sql += " OR o.usuarioId IN (SELECT usuarioId FROM grupos_usuarios_lineas WHERE grupoUsuarioId = + " + gid3 + ")";
            }
            if (gid4) {
                sql += " OR o.usuarioId IN (SELECT usuarioId FROM grupos_usuarios_lineas WHERE grupoUsuarioId = + " + gid4 + ")";
            }
            if (gid5) {
                sql += " OR o.usuarioId IN (SELECT usuarioId FROM grupos_usuarios_lineas WHERE grupoUsuarioId = + " + gid5 + ")";
            }
            if (gid6) {
                sql += " OR o.usuarioId IN (SELECT usuarioId FROM grupos_usuarios_lineas WHERE grupoUsuarioId = + " + gid6 + ")";
            }
            if (gid7) {
                sql += " OR o.usuarioId IN (SELECT usuarioId FROM grupos_usuarios_lineas WHERE grupoUsuarioId = + " + gid7 + ")";
            }                        
            sql = mysql.format(sql, [id, gid, id, id, id])
            con.query(sql, function (err, grupos) {
                con.end();
                if (err) return done(err);
                done(null, grupos);
    ...
    post: function (oferta, done) {
        comun.getConnectionCallback(function (err, con) {
            ...
            con.query(sql, function (err, grupo) {
                ...
                // Ya no se crear seguidores al crear una oferta
